...
public class ElasticsearchSinkBaseTest {
	...
	public void testItemFailureRethrownOnInvoke() throws Throwable {
		...
		verify(sink.getMockBulkProcessor(), times(1)).add(any(DocWriteRequest.class));
	...
	public void testItemFailureRethrownOnCheckpointAfterFlush() throws Throwable {
		...
		verify(sink.getMockBulkProcessor(), times(3)).add(any(DocWriteRequest.class));
	...
	private static class DummyElasticsearchSink<T> extends ElasticsearchSinkBase<T> {
		...
		protected BulkProcessor buildBulkProcessor(final BulkProcessor.Listener listener) {
			...
			when(mockBulkProcessor.add(any(DocWriteRequest.class))).thenAnswer(new Answer<Object>() {
				@Override
				public Object answer(InvocationOnMock invocationOnMock) throws Throwable {
					// intercept the request and add it to our mock bulk request
					nextBulkRequest.add(invocationOnMock.getArgumentAt(0, DocWriteRequest.class));
						...
						if (nextBulkFailure == null) {
							...
							for (int i = 0; i < currentBulkRequest.requests().size(); i++) {
								...
								if (mockItemFailure == null) {
									...
									mockResponses[i] = new BulkItemResponse(i, DocWriteRequest.OpType.CREATE, mock(DocWriteResponse.class));
								} else {
									// the mock response for the item is failure
									mockResponses[i] = new BulkItemResponse(i, DocWriteRequest.OpType.CREATE, new BulkItemResponse.Failure("index", "type", "id", new Exception(mockItemFailure)));
								}
							}
