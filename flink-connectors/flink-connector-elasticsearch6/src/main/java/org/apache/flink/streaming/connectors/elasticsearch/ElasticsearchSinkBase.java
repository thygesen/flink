 ...
 * the use of an internal {@link BulkProcessor} to buffer multiple {@link DocWriteRequest}s before
...
public abstract class ElasticsearchSinkBase<T> extends RichSinkFunction<T> implements CheckpointedFunction {
	...
	/** The function that is used to construct multiple {@link DocWriteRequest ActionRequests} from each incoming element. */
	private final ElasticsearchSinkFunction<T> elasticsearchSinkFunction;

	/** User-provided handler for failed {@link DocWriteRequest ActionRequests}. */
	...
	/** Provided to the user via the {@link ElasticsearchSinkFunction} to add {@link DocWriteRequest ActionRequests}. */
